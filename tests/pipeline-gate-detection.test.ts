...
describe('detectGateIssues (pure)', () => {
  ...
  it('字符串 pattern 未命中返回空', () => {
    const rules: QualityGateRule[] = [{ type: 'toxicity', description: 'd', severity: 'critical', pattern: 'badword' }];
    expect(detectGateIssues('clean text', rules)).toHaveLength(0);
  });

  it('全局正则每次检测前重置 lastIndex，避免状态串扰', () => {
    const rules: QualityGateRule[] = [{ type: 'p', description: 'd', severity: 'info', pattern: /x/g }];
    // 第一次调用后应重置，第二次仍能命中
    ...
    const rules: QualityGateRule[] = [
      {
        ...
        severity: 'warning',
        detector: () => ({ type: 'custom', description: 'detected', severity: 'warning' })
      }
    ];
  ...
  it('detector 返回 falsy 不上报', () => {
    const rules: QualityGateRule[] = [{ type: 'custom', description: 'd', severity: 'info', detector: () => null }];
