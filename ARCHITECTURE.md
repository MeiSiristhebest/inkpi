...
- **Phase 2 not started (2/8):** `TerminalStudio` M/VC split (#12), `WorkflowCoordinator`/`runAgentLoop` split (#13). These are the remaining "治本" structural refactors — see Known Debt.
...
**Structural (Phase 2 — 6/8 done, 2/8 not started)**
- **Giant classes not yet split.** `WorkflowCoordinator` (~551 lines / 9 responsibilities), `TerminalStudio` (~472 / 14), and `runAgentLoop` (~350-line function) remain unsplit (Phase 2 #12 / #13). They are *behaviourally correct and fully covered by the existing suite*, but violate SRP. Splitting them is the final "治本" step; it is deferred here because blind refactoring the core agent loop / coordinator risks the green test suite the user requires, and the sandbox blocks `--coverage` verification of the per-file impact. Recommended incremental approach (verify `tsc -b` + targeted tests after each):
  - `WorkflowCoordinator` → `StageRegistry` + `GateEvaluator` (strategy) + `WorkflowExecutor` + `LedgerMerger` (pure) + `TelemetryTracer`; collapse the 10× `compatibilityMode === 'legacy-pipeline'` branches into a `WorkflowStrategy` object.
  - `TerminalStudio` → `StudioModel` (domain + `subscribe()`, zero ANSI) + `StudioView` (pure `render(model)`) + `StudioController` (`handleInput(key) → Command`).
  - `runAgentLoop` → `ContextTransformer` → `StreamInvoker` → `ToolDispatcher` → `TurnFinalizer` (the two tool-concurrency strategies in `loop.ts` / `tools.ts` must be merged into one).
- **`TerminalStudio` has no ViewModel boundary** (domain state, view state, and rendering live in one class) — sub-case of #12 above.
