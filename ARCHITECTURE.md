...
- **Giant classes decomposed along responsibility boundaries (P2-12 / P2-13, done).** The split followed the incremental path (pure functions first, then method splits, then responsibility objects), keeping the suite green at every step:
  ...
  - Behaviour is preserved exactly: `tsc -b` green, full suite green (82 files / 415 tests), dependency ratchet green, new units covered by `pipeline-workflow-strategy.test.ts`, `turn-stages.test.ts`, `concurrency.test.ts`, `deprecations.test.ts`.
...
- **`perFile: true` coverage is not enabled.** Aggregate branch coverage is **81.11%** against an 80% threshold — a margin of ~1.1 points (whole-suite: 91.55% lines / 91.55% stmts / 94.63% funcs). The per-file gate was **empirically measured** (2026-09-02, escalated shell): enabling it yields **99 threshold errors across ~55 files** — 45 branch failures plus 16/16/19 lines/functions/statements — split between pure barrel/type files (`ports/index`, `turn/turn-context`, `pipeline/workflow-types`, tui `atoms/*`, both `deprecations.ts`, 0–50% branch, inherently mismatched to a per-file floor) and core logic files sitting at ~70% (`branch-what-if` 58.8% lowest). Landing it needs a dedicated coverage-engineering pass (exempt pure-declaration barrels, then add branch assertions for the ~70% core files) rather than a config flip; it is tracked as such in `ARCHITECTURE_REVIEW.md` #20.
...
- **Lint:** `pnpm lint` → `biome check`. ✅ Clean — zero diagnostics across `packages tests scripts` (410 files); the historical invalid-config debt was cleared as a standalone commit.
