...
- **Coverage Gate**: `vitest.config.ts` enforces $\ge 85\%$ lines, $\ge 85\%$ statements, $\ge 85\%$ functions, and $\ge 75\%$ branches. These are **aggregate** thresholds — `perFile` is not enabled, so local gaps are not represented by the aggregate gate (latest full run: 89.91% lines, 89.91% statements, 93.12% functions, 79.14% branches).
...
This section records what changed so the documentation stays honest about the code. The executable status matrix is maintained in `docs/specs/AI_RUNTIME_SPEC_v1.md`.
...
- **RPC `withSession()` guard + generic workflow strategy (P2-14, done).** The "get session / not-found throw" boilerplate repeated 9× across `InkPiDaemon`'s method handlers is a single `withSession()` helper; the `@inkpi/server` RPC layer uses a `registerMethod()` registry (no 245-line `switch`). On the pipeline side, `WorkflowCoordinator` exposes one domain-neutral strategy (`pipeline/workflow-strategy.ts`); the legacy compatibility mode, stage-name hooks, and direct pipeline RPC gateway have been removed.
...
- **Giant classes decomposed along responsibility boundaries (P2-12 / P2-13, done).** The split followed the incremental path (pure functions first, then method splits, then responsibility objects), keeping the suite green at every step:
  - `WorkflowCoordinator` (was ~660 lines / 9 responsibilities) is now a ~215-line assembly facade over collaborators in `pipeline/`: `WorkflowExecutor` (stage loop orchestration, 353 lines), `WorkflowStrategy` (the domain-neutral strategy, see P2-14), `StageRegistry` (stage list + lookup), `GateRuleRegistry` (global + per-stage gate rules), `GateEvaluator` via pure `detectGateIssues()` (unit-tested), `RoleInvoker` (role invocation + error semantics), `TelemetryTracer` (span lifecycle), `EventBus` (event fan-out), and pure `mergeLedgers`/`mergeRecords` in `ledger-merge.ts`. The class keeps its public name `WorkflowCoordinator` and exposes only `runWorkflow()`.
  ...
  - Behaviour is preserved exactly: `tsc -b` green, full suite green (169 files / 779 tests), dependency ratchet green, new units covered by `pipeline-workflow-strategy.test.ts`, `turn-stages.test.ts`, `concurrency.test.ts`, `deprecations.test.ts`.
...
- **Phase 3 follow-up (1/6):** the `perFile` coverage gate (#20) remains disabled. The aggregate gate is enforced; enabling a per-file floor requires a separate coverage-engineering pass (see Tooling below).
...
- **Retained with rationale (name carries meaning, not an empty-generic suffix):** the `Runner` group (`ExtensionRunner` — loads & runs extension modules; `ClipboardCommandRunner` — executes clipboard CLI commands; the interface now `SandboxRunner`), `TerminalHarness` (a terminal test/embed harness). The rationale is recorded here and in the Runtime v1 specification.
- **`Novel*` business prefix: EVALUATED, retained as documented packaging debt.** The generic workflow coordinator no longer embeds narrative stages or legacy aliases. Narrative-specific gate helpers remain opt-in under `pipeline/narrative-gates.ts`; `compaction/state-ledger.ts` still exports `extractNovelStateLedger` / `formatNovelStateLedger`, and `@inkpi/evals` ships a `NovelEvalRunner`. Moving those helpers out requires a new domain package (e.g. `@inkpi/novel-pipeline`) that consumes the generic core — a packaging change, not a rename.
...
- **`perFile: true` coverage is not enabled.** The current aggregate report is 79.14% branches, 89.91% lines, 89.91% statements, and 93.12% functions against the thresholds in `vitest.config.ts`. A per-file floor remains a separate coverage-engineering task: declaration-only barrels and under-covered core branches need an explicit policy and dedicated tests before the config can be changed. It is tracked in the Phase 23 checklist in `docs/specs/AI_RUNTIME_SPEC_v1.md`.
...
- **`pnpm lint` is clean.** The historical debt — an invalid `biome.json` key (`includes` instead of `include`) that made Biome exit before checking anything, plus ~138 lint-rule violations and ~136 formatting diffs accrued on top — was cleared in one standalone commit (`biome format --write` + a reviewed `biome check --write`, with manual review of every non-mechanical fix). `biome check packages tests scripts` now reports **zero diagnostics across 420 files**. Semantics were preserved: the only `delete → = undefined` rewrites the unsafe fixer proposed were kept solely where provably equivalent (`process.env`, JSON-serialised output) and restored with a documented `biome-ignore` where the key must actually disappear (`mergeLedgers` legacy-alias stripping, env teardown in tests); four pure-static classes (`TerminalMermaid` / `StoryboardExporter` / `PromptCacheOptimizer` / `SessionShareExporter`) were converted to const objects since nothing ever `new`ed or subtyped them.
...
- **Test:** `pnpm test` → `vitest run`; the latest full run covered 169 test files and 779 tests.
- **Coverage:** `pnpm test:coverage` → `vitest run --coverage`, with aggregate thresholds of 85% lines / 85% statements / 85% functions / 75% branches.
- **Lint:** `pnpm lint` → `biome check`. ✅ Clean — zero diagnostics across `packages tests scripts` (420 files); the historical invalid-config debt was cleared as a standalone commit.
