...
/** Runtime-owned tool names for the four first-party tool plugins. */
export const FIRST_PARTY_RUNTIME_TOOL_NAMES = [
...
/** Runtime-owned workflow task kinds for the two first-party workflow plugins. */
export const FIRST_PARTY_RUNTIME_WORKFLOW_KINDS = [
...
/**
 * Registers the six plugins that cannot be represented by a plain Desktop
 * view in the Runtime-owned registries. The executable functions stay in the
 * daemon process; only their names and task results cross the RPC boundary.
 *
 * Registration is idempotent with respect to an already-owned name. Existing
 * host registrations are never overwritten by this first-party bootstrap.
 */
export function registerFirstPartyPluginRuntime(options: {
  ...
  for (const tool of createFirstPartyTools()) {
  ...
  for (const handler of createFirstPartyWorkflowHandlers()) {
  ...
  return {
    toolNames: [...FIRST_PARTY_RUNTIME_TOOL_NAMES],
    workflowKinds: [...FIRST_PARTY_RUNTIME_WORKFLOW_KINDS],
    dispose: () => {
...
function createFirstPartyTools(): AgentTool[] {
  return [
    {
      name: 'plugin.diff-reviewer.compute',
      label: 'Diff Reviewer',
    ...
    {
      name: 'plugin.memory-palace.search',
      label: 'Memory Palace',
    ...
    {
      name: 'plugin.press-forge.format',
      label: 'Press Forge',
    ...
    {
      name: 'plugin.scrapbook-recycler.recommend',
...
function createFirstPartyWorkflowHandlers(): TaskHandler[] {
  return [
    createWorkflowHandler('multiverse-whatif', 'plugin.multiverse-whatif.workflow', (input) =>
      simulateMultiverse(input)
    ),
    createWorkflowHandler('storyboard-gen', 'plugin.storyboard-gen.workflow', (input) => extractStoryboard(input))
  ];
}
