...
export class ModelCatalogManager {
  ...
  public filterByCapability(capability: { thinking?: boolean; tools?: boolean; vision?: boolean }): ModelCatalogEntry[] {
  ...
  public getRecommendedPlanningModel(): ModelCatalogEntry {
    ...
    const preferred = candidates.find((m) =>
      m.id.includes('r1') || m.id.includes('3.7') || m.id.includes('o3') || m.id.includes('gemini-2.5-pro')
    );
