...
describe('daemon domain change materializer', () => {
  it('materializes Desktop and daemon field names into the derived read models', () => {
    ...
    expect(JSON.parse(String((db.prepare('SELECT metadata FROM workspaces WHERE id = ?').get('project-1') as { metadata: string }).metadata))).toEqual({
      projectType: 'full',
      features: ['timeline'],
    });
    ...
    const protocolFields = makeChangeSet('set-2', 'project-1', 1, [
      change('document-change', 'document', 'chapter-1', 'upsert', {
        id: 'chapter-1',
  ...
  it('deletes dependents in foreign-key order for document, folder, and workspace deletes', () => {
    ...
    projection.apply(
      makeChangeSet('set-1', 'workspace-1', 0, [
        change('workspace-change', 'workspace', 'workspace-1', 'upsert', { id: 'workspace-1', title: 'W', owner: 'owner', createdAt: 1, updatedAt: 1 }),
        change('folder-change', 'folder', 'folder-1', 'upsert', { id: 'folder-1', workspaceId: 'workspace-1', title: 'F', createdAt: 1, updatedAt: 1 }),
        change('document-change', 'document', 'document-1', 'upsert', {
          id: 'document-1', workspaceId: 'workspace-1', folderId: 'folder-1', title: 'D', contentMarkdown: 'text', createdAt: 1, updatedAt: 1,
        }),
      ]),
    );
    db.prepare('INSERT INTO document_deltas (document_id, step_json, client_timestamp, created_at) VALUES (?, ?, ?, ?)').run('document-1', '{}', 1, 1);
    db.prepare('INSERT INTO lanes (id, workspace_id, name, created_at, updated_at) VALUES (?, ?, ?, ?, ?)').run('lane-1', 'workspace-1', 'default', 1, 1);
    db.prepare('INSERT INTO branch_tips (lane_id, document_id, head_snapshot_version, updated_at) VALUES (?, ?, ?, ?)').run('lane-1', 'document-1', 1, 1);
  ...
  it('rebuilds derived rows from a restored authoritative snapshot', () => {
    ...
    const first = makeChangeSet('set-1', 'workspace-1', 0, [
      change('workspace-change', 'workspace', 'workspace-1', 'upsert', { id: 'workspace-1', title: 'Original', owner: 'owner', createdAt: 1, updatedAt: 1 }),
      change('folder-change', 'folder', 'folder-1', 'upsert', { id: 'folder-1', workspaceId: 'workspace-1', title: 'Folder', createdAt: 1, updatedAt: 1 }),
      change('document-change', 'document', 'document-1', 'upsert', { id: 'document-1', workspaceId: 'workspace-1', folderId: 'folder-1', title: 'Document', contentMarkdown: 'content', createdAt: 1, updatedAt: 1 }),
    ]);
    ...
    db.prepare('INSERT INTO folders (id, workspace_id, title, order_index, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)').run('stale-folder', 'workspace-1', 'stale', 99, 1, 1);

    projection.restoreSnapshot(snapshot);
    expect((db.prepare('SELECT title FROM workspaces WHERE id = ?').get('workspace-1') as { title: string }).title).toBe('Original');
