...
export function parseSkillMetadata(content: string, filePath: string): SkillInfo | null {
  ...
  if (endIdx === -1) return null;
  const frontmatter: Record<string, unknown> = {};
  for (const line of trimmed.slice(3, endIdx).trim().split('\n')) {
    const colonIdx = line.indexOf(':');
    if (colonIdx === -1) continue;
    frontmatter[line.slice(0, colonIdx).trim()] = line.slice(colonIdx + 1).trim();
  }
  const name = String(frontmatter.name || basename(filePath).replace(/\.(md|skill\.md)$/, ''));
  return { name, description: String(frontmatter.description || ''), filePath, frontmatter, promptBody: '' };
}
...
export class SkillDiscoveryEngine {
      ...
      for (const entry of entries) {
        ...
        if (stat.isDirectory()) {
          ...
          try {
            ...
            const skill = parseSkillMetadata(content, fullPath);
            if (skill) {
  ...
  public loadSkill(name: string): SkillInfo | undefined {
    ...
    if (!metadata || metadata.promptBody) return metadata;
    try {
